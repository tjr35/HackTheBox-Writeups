...
Viewing the website, it seems to be some form of recruiting site for developers to both find jobs and for companies to find developers:
![Pasted image 20260402132432](Pasted%20image%2020260402132432.png)

None of the buttons on the site seem to lead anywhere. Viewing a 404 error page, leaks that Apache 2.4.58 is in use:
![Pasted image 20260402132626](Pasted%20image%2020260402132626.png)
...
![Pasted image 20260402132923](Pasted%20image%2020260402132923.png)
...
When connecting initially to the FTP server with a dummy username, it states "This FTP server is anonymous only." This also shows that vsFTPd 3.0.5 is in use:
![Pasted image 20260402133148](Pasted%20image%2020260402133148.png)
...
Inside the ftp share is a single folder `pub` with a single `.jar` file called `employee-service.jar`, which I download for later analysis:
![Pasted image 20260402133335](Pasted%20image%2020260402133335.png)
...
Viewing this in the browser shows a 404 error, also disclosing Jetty is in use:
![Pasted image 20260402134821](Pasted%20image%2020260402134821.png)
...
#### Port 8500
Scanning port 8500 with nmap, returns a slightly garbled response signifying it _might_ be a golang http server. Visiting the port directly we get the following message:
![Pasted image 20260402135234](Pasted%20image%2020260402135234.png)

I try using the proxy to connect to one of the web servers, but it fails, requiring authentication:
![Pasted image 20260402142145](Pasted%20image%2020260402142145.png)
#### Port 8888
...
Visiting this port directly in the browser we get another web page **Hoverfly** with a login page, documentation and a github, suggesting this is an off the shelf product:
![Pasted image 20260402135454](Pasted%20image%2020260402135454.png)
...
Digging into the files, we notice this appears to be the code for the server running on port 8080, and it gives hints to some potentially useful endpoints:
![Pasted image 20260402140246](Pasted%20image%2020260402140246.png)

This appears to be a few simple SOAP APIs that are used for submitting reports, we can view the documentation for the APIs at `http://devarea.htb:8080/employeeservice?wsdl`:
![Pasted image 20260402140438](Pasted%20image%2020260402140438.png)

As these are SOAP XML APIs I attempt some XXE/XML injection but to no avail:
![Pasted image 20260402141252](Pasted%20image%2020260402141252.png)

At this stage I have an AI agent analyse the code for me, which extracts that Apache CXF 3.2.14 is in use which is vulnerable to CVE-2022-46364. We can confirm this version of apache is in use by looking at the pom.xml:
![Pasted image 20260402143329](Pasted%20image%2020260402143329.png)
...
This returns a base64 encoded `/etc/passwd`:
![Pasted image 20260402143615](Pasted%20image%2020260402143615.png)

Which decodes to:
![Pasted image 20260402143652](Pasted%20image%2020260402143652.png)
...
![Pasted image 20260402145232](Pasted%20image%2020260402145232.png)
### Gaining a shell
...
This grants us a shell as dev_ryan and we can grab the user shell:
![Pasted image 20260402145512](Pasted%20image%2020260402145512.png)
...
In the home directory there is a file named `syswatch-v1.zip` which is unusual. Checking the sudo permissions for dev_ryan returns:
![Pasted image 20260402160645](Pasted%20image%2020260402160645.png)
...
Bizarrely, whether misconfigured or otherwise, we have write permission over `/bin/bash`. As `/bin/bash` is called at the top of our syswatch script, we should be able to overwrite this with a malicious binary in order to gain a shell:
![Pasted image 20260402162529](Pasted%20image%2020260402162529.png)
### Exploit PoC
...
After doing all these steps we can elevate to root:
![Pasted image 20260402170104](Pasted%20image%2020260402170104.png)

And get the root flag:
![Pasted image 20260402170134](Pasted%20image%2020260402170134.png)
