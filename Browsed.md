...
The site appears to be built for the sharing of browser extensions, with uploads in zip format, with a note that files must be placed directly inside the archive and not in a subfolder. ![Pasted image 20260330134040](Screenshots/Pasted%20image%2020260330134040.png)

The site also mentions Chrome version 134. There is mention that a developer will review uploaded extensions and respond with feedback. ![Pasted image 20260330134013](Screenshots/Pasted%20image%2020260330134013.png)

Navigating to `/samples.html` reveals some example extensions available for download. ![Pasted image 20260330134056](Screenshots/Pasted%20image%2020260330134056.png)

I test the upload functionality by downloading one of the sample extensions and uploading it, which returns a very verbose log: ![Pasted image 20260330134230](Screenshots/Pasted%20image%2020260330134230.png)
...
![Pasted image 20260330134544](Screenshots/Pasted%20image%2020260330134544.png)
...
![Pasted image 20260330141424](Screenshots/Pasted%20image%2020260330141424.png) No additional subdomains were found.
...
It finds that on the backend when we upload an extension it is being loaded into `Chrome for Testing` running headlessly under `/var/www`. ![Pasted image 20260330135034](Screenshots/Pasted%20image%2020260330135034.png)

It also identifies a potential gitea instance on `http://browsedinternals.htb` which is being called out to by the browser during the execution and responses are being returned, suggesting it is allowed to make authenticated requests. ![Pasted image 20260330135102](Screenshots/Pasted%20image%2020260330135102.png)

It also highlights that gitea 1.24.5 is likely in use and that SSRF or XSS is likely our way forward. ![Pasted image 20260330135117](Screenshots/Pasted%20image%2020260330135117.png)

---

## 2. Browsedinternals Enumeration

Knowing that there is likely another host, `browsedinternals.htb`, I add this to my `/etc/hosts` and see if it is externally accessible. It resolves and shows the default gitea web page: ![Pasted image 20260330135536](Screenshots/Pasted%20image%2020260330135536.png)

Navigating to explore, we see a singular repository by a user named **larry**: ![Pasted image 20260330135627](Screenshots/Pasted%20image%2020260330135627.png)

Viewing this repo, it appears to be a flask application running internally on port 5000. It is running on `127.0.0.1` and so should only be accessible via localhost: ![Pasted image 20260330135727](Screenshots/Pasted%20image%2020260330135727.png)
...
```bash
if [ "$1" -eq 0 ](%20"$1"%20-eq%200%20); then
```
...
I then zip the extension back up and upload it. And we get a hit on our listener: ![Pasted image 20260330141513](Screenshots/Pasted%20image%2020260330141513.png)
...
I then zip this back up and upload, with a penelope listener set up to catch the shell. We get a hit and a shell as `larry` as well as the user flag: ![Pasted image 20260330143326](Screenshots/Pasted%20image%2020260330143326.png)
...
Viewing this file, it appears to be a script which takes an extension in one of the directory names in `extensions` and performs a few actions on it, such as bumping versions: ![Pasted image 20260330143710](Screenshots/Pasted%20image%2020260330143710.png)

Inside `/opt/extensiontool/` the `__pycache__` folder is world writable, allowing any user to write files here: ![Pasted image 20260330143451](Screenshots/Pasted%20image%2020260330143451.png)
...
And with this we can get the root flag: ![Pasted image 20260330154959](Screenshots/Pasted%20image%2020260330154959.png)
